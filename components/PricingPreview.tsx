...
export function PricingPreview({ showExclusions = false }: PricingPreviewProps) {
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {prices.map((item) => (
        <article key={item.title} className="rounded-md border border-white/10 bg-white/[0.03] p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/45">{item.tag}</p>
          <h3 className="mt-4 text-xl font-semibold text-white">{item.title}</h3>
          <p className="mt-5 text-5xl font-semibold tracking-normal text-white">{item.price}</p>
          <p className="mt-2 text-sm text-white/50">Flat fee. Attorney reviewed. Personalized drafting</p>
